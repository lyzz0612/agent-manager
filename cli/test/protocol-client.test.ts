...
describe('protocol client URL helpers', () => {
  it('builds the login URL with no double slash', () => {
    assert.equal(
      ...
      'https://server.test/api/runner/login',
    );
    assert.equal(
      buildLoginUrl('https://server.test'),
      'https://server.test/api/runner/login',
  ...
  it('builds a wss URL for https servers and embeds the token', () => {
    ...
    assert.match(url, /^wss:\/\/server\.test\/ws\/runner\?runnerToken=abc$/);
  });
  ...
  it('builds a ws URL for http servers', () => {
    ...
    assert.match(
      url,
      /^ws:\/\/localhost:3000\/base\/ws\/runner\?runnerToken=tok$/,
    );
  });
});
  ...
  it('returns the parsed response on success', async () => {
    const fakeFetch: typeof fetch = async () =>
      new Response(
        JSON.stringify({
          runnerToken: 'token',
          machine: { id: 'm_xx', displayName: 'box' },
        }),
